...
{\bfseries BENCHMARKING CALIBRATED GRADIENT BOOSTING WITH BEHAVIOURAL
FEATURES FOR TRANSACTION-LEVEL AML DETECTION}

{\bfseries \tsp{1}T.K.
Azbergen}\fig{i2/image1}[]{\bfseries ,
\tsp{2}A.O.Tleubayeva}\fig{i2/image1}[]{\bfseries \envelope ,
\tsp{1}A.Aituarov}\fig{i2/image1}[]{\bfseries ,
\tsp{1}Y.T.}
{\bfseries Sabyrzhan}\fig{i2/image1}[]

\emph{\tsp{1}Kazakh-British Technical University, Almaty,
Kazakhstan,}

\emph{\tsp{2}Astana IT University, Astana, Kazakhstan}

\envelope \href{mailto:Corresponding\%20author:\%20}{\emph{Corresponding
author:}} \emph{Arailym.tll@gmail.com}
...
probabilistic calibration and behavioural feature modelling on
...
transactions simulating realistic banking activity.

Model performance is assessed using ranking-, calibration-, and
operationally oriented metrics, including AUPRC, AUROC, Brier score,
...
{\bfseries СРАВНИТЕЛЬНАЯ ОЦЕНКА КАЛИБРОВАННОГО ГРАДИЕНТНОГО БУСТИНГА С
ПОВЕДЕНЧЕСКИМИ ПРИЗНАКАМИ ДЛЯ ВЫЯВЛЕНИЯ ОТМЫВАНИЯ ДЕНЕГ НА УРОВНЕ
ТРАНЗАКЦИЙ}

{\bfseries \tsp{1}Т.К.Азберген, \tsp{2}А.О.
Тлеубаева\envelope , \tsp{1}А.Т.Айтуов,
\tsp{1}Е.Т.Сабыржан}

\emph{\tsp{1}Казахстанско-Британский технический
университет, Алматы, Казахстан}

\emph{\tsp{2}Astana IT University, Астана, Казахстан}

\emph{e-mail:
Arailym.tll@gmail.com}
...
{\bfseries ТРАНЗАКЦИЯЛАР ДЕҢГЕЙІНДЕ АҚШАНЫ ЖЫЛЫСТАТУДЫ АНЫҚТАУҒА АРНАЛҒАН
МІНЕЗ-ҚҰЛЫҚТЫҚ БЕЛГІЛЕРГЕ НЕГІЗДЕЛГЕН КАЛИБРЛЕНГЕН ГРАДИЕНТТІ БУСТИНГТІҢ
САЛЫСТЫРМАЛЫ БАҒАЛАНУЫ}

{\bfseries \tsp{1}Т.К.Азберген,
\tsp{2}А.О.Тлеубаева\envelope ,
\tsp{1}А.Т.Айтуов, \tsp{1}Е.Т.Сабыржан}
...
базалық модельдер арасында калибрленген CatBoost бағаланған базалық
...
learning{\bfseries ,~}probability calibration, and~graph-based analysis.
...
1. Platt Scaling (Sigmoid Calibration){\bfseries ~-} fits a logistic
...
nodes
representing~accounts~through~edges~representing~transactions
{[}22{]}.
...
\(\text{AUROC} = \int_{0}^{1}{TPR(FPR)}\, dFPR\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \)(3)

where:

\(TPR = \frac{TP}{TP + FN},\quad FPR = \frac{FP}{FP + TN}\) (4)
...
\(\text{PrecisionTopk} = TP_{k}TPk + FPk\) (6)
...
\(\quad P_{\text{base}} = \frac{TP + FP}{N}\) (8)
...
{\bfseries Table 1 \emph{-} Performance comparison of baseline models on
the IBM-AML dataset}

%% \begin{longtable}[]{@{}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2623}}
%%   >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1055}}
%%   >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1095}}
%%   >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0879}}
%%   >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1733}}
%%   >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1432}}
...
%% {\bfseries CatBoost (cal.)} & {\bfseries 0.3674} & {\bfseries 0.959} &
%% {\bfseries 0.0062} & {\bfseries 0.285} & {\bfseries 0.409} & {\bfseries 40.9×} \\
%% Graph Neural Network (EdgeSAGE) & 0.0110 & 0.666 & 0.772 & 0.007 & 0.009
%% & 0.9× \\
%% \end{longtable}
...
{\bfseries Table 2} \emph{-} {\bfseries Effect of probability calibration on
CatBoost performance}

%% \begin{longtable}[]{@{}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.3054}}
...
%%   >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1904}}
...
%% Platt (Sigmoid) Calibration & {\bfseries 0.3674} & {\bfseries 0.0062} &
%% {\bfseries 0.285} & {\bfseries 0.409} & {\bfseries 40.9×} \\
%% Isotonic Calibration & 0.3439 & {\bfseries 0.0054} & 0.281 & 0.403 &
%% 40.4× \\
%% \end{longtable}
...
{\bfseries Fig.1 \emph{-}} {\bfseries AUPRC by model on the IBM-AML dataset}

\fig{i2/image3}[]

{\bfseries Fig.2 \emph{-}} {\bfseries Brier Score by model}

\fig{i2/image4}[]

{\bfseries Fig.3 \emph{-}} {\bfseries Combined reliability diagram for all
models}
...
{\bfseries Table 3} \emph{-} {\bfseries Ablation results for behavioural
features}

%% \begin{longtable}[]{@{}
%%   >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2408}}
%%   >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1231}}
%%   >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1358}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5003}}@{}}
%% \toprule\noalign{}
%% \begin{minipage}[b]{\linewidth}\centering
%% {\bfseries Removed Feature Group}
%% \end{minipage} & \begin{minipage}[b]{\linewidth}\centering
%% {\bfseries ΔAUPRC}
%% \end{minipage} & \begin{minipage}[b]{\linewidth}\centering
%% {\bfseries ΔLift@1\%}
...
%% \bottomrule\noalign{}
%% \endlastfoot
%% 1-Day Rolling Window & −0.012 & −2.0× & Short-term volatility features
%% improve responsiveness to rapid behaviour changes. \\
%% MAD (Mean Absolute Deviation) & −0.008 & −1.5× & Dispersion-based
%% features enhance anomaly sensitivity in low-signal regions. \\
%% Both (1-Day + MAD) & {\bfseries −0.023} & {\bfseries −4.0×} & Combined removal
%% leads to noticeable degradation in ranking precision. \\
%% \end{longtable}
...
{\bfseries Table 4} -~{\bfseries Monthly backtesting results (AUPRC) on the
IBM-AML dataset}
...
{\bfseries Fig.} {\bfseries 4 \emph{-}} {\bfseries Temporal backtesting results
